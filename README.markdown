...
- **Связь**: ESP-NOW для загрузки UI JSON с ESP32-S3-N16R8.
- **Файловая система**:
  - **карта памяти - FAT32** (MIN 8GB - 32GB MAX): /system (~1GB), /boot (64MB), /tmp (10MB), /vendor (~1GB), /sdcard (~).


### Упрощённая файловая система
Эмулирует Wear OS 6 FS:
- **/** (32GB): Сдесь и будут разделы ввиде папок ссылок (/boot, /system и т.д)
- **/system** (~ 1GB): UI, конфиги, watchfaces (e.g., /system/fonts/Android.json).
- **/boot** (max 64MB): Анимация запуска (/boot/bootanimation.json или .bin для LVGL), OTA-метаданные.
- **/sdcard** (max 32GB, SD): Пользовательские данные (логи, настройки, и т.д).
- **/tmp** (~10MB): Логи (энкодер, сенсор, батарея, и т.д).
- **/vendor** (~1GB): Конфиги и драйвера hardware (ST7789, XPT2046, энкодер и т.д).
- **/dev** (~): Устрйстава
